...
-- creator 1 is the demo's "you" persona (a working creator); creator 2 is a
-- plain admin account used for the Approvals/Submissions views.
INSERT INTO creators (name, email, is_admin) VALUES
  ('Tylor C.', 'tylor.clont@gmail.com', 0),
  ('Admin', 'admin@managedops.demo', 1),
  ('Sam Okafor', 'sam@managedops.demo', 0),
  ('Priya Natarajan', 'priya@managedops.demo', 0);

-- 1: Acme Corp / daily / claimed by Tylor C., with completed history
INSERT INTO managed_ops (client_id, task_type, cadence_type, cadence_config, status, claimed_by, claimed_at) VALUES
  (1, 'Client Profile Refresh', 'daily', '{}', 'claimed', 1, datetime('now', '-3 days'));

-- 2: Bluebird Media / weekly / claimed by Sam, with completed history
INSERT INTO managed_ops (client_id, task_type, cadence_type, cadence_config, status, claimed_by, claimed_at) VALUES
  (2, 'Social Listing Sync', 'weekly', '{}', 'claimed', 3, datetime('now', '-21 days'));
...
INSERT INTO managed_ops (client_id, task_type, cadence_type, cadence_config, status, claimed_by, claimed_at, pending_drop_request) VALUES
  (3, 'Directory Content Audit', 'monthly', '{}', 'claimed', 4, datetime('now', '-40 days'), 1);

-- 4: DriftCo / every 3 days / claimed by Sam, overdue cycle (safety-net demo)
INSERT INTO managed_ops (client_id, task_type, cadence_type, cadence_config, status, claimed_by, claimed_at) VALUES
  (4, 'Inventory Update', 'every_n_days', '{"interval":3}', 'claimed', 3, datetime('now', '-10 days'));
...
-- 6: Harbor Fitness / every 5 days / unclaimed
INSERT INTO managed_ops (client_id, task_type, cadence_type, cadence_config, status) VALUES
  (6, 'Member Directory Sync', 'every_n_days', '{"interval":5}', 'unclaimed');

-- Op 1 (Acme Corp, daily): two completed cycles + one open pending cycle due today
INSERT INTO op_cycles (managed_op_id, due_date, status, completed_at, completed_by) VALUES
  (1, date('now', '-2 days'), 'done', datetime('now', '-2 days', '+3 hours'), 1),
  (1, date('now', '-1 days'), 'done', datetime('now', '-1 days', '+2 hours'), 1);
...
INSERT INTO op_cycles (managed_op_id, due_date, status, completed_at, completed_by) VALUES
  (2, date('now', '-14 days'), 'done', datetime('now', '-14 days', '+2 hours'), 3),
  (2, date('now', '-7 days'), 'done', datetime('now', '-7 days', '+3 hours'), 3);
INSERT INTO op_cycles (managed_op_id, due_date, status) VALUES
  (2, date('now'), 'pending');

-- Op 3 (Coastal, monthly): one completed cycle + one open pending cycle a few weeks out
INSERT INTO op_cycles (managed_op_id, due_date, status, completed_at, completed_by) VALUES
  (3, date('now', '-10 days'), 'done', datetime('now', '-10 days', '+1 hours'), 4);
INSERT INTO op_cycles (managed_op_id, due_date, status) VALUES
...
INSERT INTO op_cycles (managed_op_id, due_date, status, completed_at, completed_by) VALUES
  (4, date('now', '-5 days'), 'done', datetime('now', '-5 days', '+4 hours'), 3);
INSERT INTO op_cycles (managed_op_id, due_date, status) VALUES
  (4, date('now', '-2 days'), 'pending');

-- Audit log
INSERT INTO claim_events (managed_op_id, creator_id, action, created_at) VALUES
  (1, 1, 'claimed', datetime('now', '-3 days')),
  (2, 3, 'claimed', datetime('now', '-21 days')),
  (3, 4, 'claimed', datetime('now', '-40 days')),
  (4, 3, 'claimed', datetime('now', '-10 days'));

INSERT INTO claim_events (managed_op_id, creator_id, action, note, created_at) VALUES
  (3, 4, 'drop_requested', 'Client relationship winding down, reassigning workload before offboarding.', datetime('now', '-1 days'));
